...
function cJS(){
	...
	this.defaultProxyHandler = {
		set: function(target, property, value, receiver) {
			...
			if(target.watchers && target.watchers.length > 0){
				for (var i = 0; i < target.watchers.length; i++){
					if(target.watchers[i].fieldname == property){
						target.watchers[i].handler(target, property, value);
					}
				}
			}
		}	
			...
			for(var i=0;i<bindings.length;i++){							
				...
				if(binding.name == fieldName || !fieldName){								
					for(var j = 0; j < binding.elements.length; j++){
						let element = binding.elements[j];
