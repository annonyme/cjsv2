	...
	this.addController = function(name, controller, containingElement, initMethod, services){
		...
		let addWatcher = function(name){
			return function(fieldname, handler){
				cjs.getController(name).watchers.push({
					fieldname: fieldname,
					handler: handler
				});
			}
	...
	this.findValueBindings=function(controllerWrapper,containingElement){
		...
		for(var i=0;i<elements.length;i++){
			...
			if(valueName.match(/:/)){
				...
				if(element.getAttribute('data-cjs-template') && Mustache) {
					type="template";
					template = element.getAttribute('data-cjs-template');
				}
			}
					...
					binding.elements.forEach((element) => {
						if(element){
							...
							if(element.type === "value"){
								...
								let temp = document.getElementById(element.template).innerHTML;
								element.element.innerHTML = Mustache.render(temp, { data: wrapper.instance[binding.name] });
							}
							else{
